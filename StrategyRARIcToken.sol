...
contract Strategy is BaseStrategy {
    ...
    // 1)  Conversions from want to cToken or fcToken require a decimal update (want has 18, cToken and fcToken have 8)
    //      want -> cToken (-10 decimals), cToken -> want (+10 decimals)
    ...
    //      ETH -> want -> cToken -> fcToken (Biggest to Smallest)
    ...
    function estimatedTotalAssets() public view override returns (uint256) {
        ...
        return fcToken.balanceOf(address(this)).mul(10e46).div(fcToken.exchangeRateCurrent()).div(cToken.exchangeRateCurrent());
    }

    function domath() public view returns(uint){
    ...
    function prepareReturn(uint256 _debtOutstanding) internal override returns(uint256 _profit, uint256 _loss, uint256 _debtPayment){
        ...
        _profit = estimatedTotalAssets().sub(StampBalance); // _profit is in want
        fcToken.redeemUnderlying(_profit.add(_debtOutstanding).mul(cToken.exchangeRateCurrent()).div(10e18)); // Multiply then remove 18 decimals (big to small, Rule 2)
        cToken.redeemUnderlying(_profit.add(_debtOutstanding));

        StampBalance = estimatedTotalAssets().sub(_debtOutstanding);
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        if(StampBalance == 0){StampBalance = estimatedTotalAssets();}
            ...
            StampBalance = StampBalance.add((_debtOutstanding.mul(fcToken.exchangeRateCurrent()).mul(cToken.exchangeRateCurrent()).div(10e36)));
        }
    }
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        ...
        fcToken.redeemUnderlying(_amountNeeded.mul(cToken.exchangeRateCurrent()).div(10e18)); // Multiply then remove 18 decimals (big to small)
        cToken.redeemUnderlying(_amountNeeded); // redeem cToken for want

        StampBalance = StampBalance.sub(_amountNeeded);
    ...
    function liquidateAllPositions() internal override returns (uint256) {
        ...
        cToken.redeem(cToken.balanceOf(address(this))); // redeem all cToken for want
